...
classDiagram
    ...
    class TransferQueueDataPlane {
        ...
        +build_streaming_dataset(partition_id, required_fields, batch_size, sampler)
...
TrainerWorker 完成 train_k
  ...
  -> TransferQueueDataPlane.clear_partition(train_k)
```
...
### 7.1 开发者 A：TransferQueueDataPlane 与数据协议
...
class TransferQueueDataPlane:
    ...
    def put_rollout_batch(self, partition_id: str, samples: list[dict]): ...
    def claim_reward_batch(self, partition_id: str, batch_size: int): ...
    def append_rewards(self, metadata, rewards): ...
    def claim_reward_ready_groups(self, partition_id: str, num_generations: int, max_groups: int): ...
    def append_advantages(self, group_metadata, advantages): ...
    def build_streaming_dataset(self, partition_id: str, required_fields: list[str], batch_size: int, sampler): ...
...
### 7.2 开发者 B：AsyncRollouter、StalenessManager 与多轮交互
...
```python
class StalenessManager:
    def get_rollout_capacity(self, metadata: QueueMetadata, policy_version: int) -> RolloutCapacity: ...
...
```text
BaseRLPipeline.sync_weights(train_k)
  -> CheckpointEngineManager
  -> vLLMSampler.receive_weights()
  -> TransferQueueDataPlane.clear_partition(train_k)
```
