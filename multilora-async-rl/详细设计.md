...
```text
BaseRLPipeline:
  训练编排入口，负责初始化组件、创建角色、驱动主循环、管理一个或多个 LoraContext，
...
policy_version / train_k / partition 的关系:
  ...
  一个 partition 可以累积多个 prompt group，直到 ready_groups >= rollout.batch_size。
...
classDiagram
    class BaseRLPipeline {
        ...
        +list~LoraContext~ contexts
        +LoraAdapterRegistry adapter_registry
...
samples_per_partition =
  rollout.batch_size * rollout.num_generations
```
...
transfer_queue:
  ...
  capacity:
    ...
    safety_factor: 1.2
    max_rows: auto                    # rollout.batch_size * rollout.num_generations * (max_staleness + 1)
...
rollout:
  class: MultiTurnRollout
  batch_size: 128                    # 一个 train_k 目标收集多少个 prompt group
  max_turns: 6
  num_generations: 8
...
| 参数 | 含义 |
|---|---|
| `rollout.batch_size` | 一个 `train_k` 目标收集的 prompt group 数 |
...
| `transfer_queue.capacity.max_rows` | TQ 初始化 row 容量，默认按 `rollout.batch_size * rollout.num_generations * (max_staleness + 1)` 计算 |
...
- 实现 TQ 容量初始化：`rollout.batch_size * rollout.num_generations * (max_staleness + 1)`。
...
classDiagram
    class AsyncRollouter {
        +TransferQueueDataPlane data_plane
        +LoraAdapterRegistry adapter_registry
        +StalenessManager staleness_manager
        +RolloutPolicy rollout_policy
        +ToolManagerFactory tool_manager_factory
        +dict reward_registry

        +int max_concurrency
        +int default_partition_groups
        +dict partition_groups_by_context

        +dict pending_groups_by_context
        +set active_tasks
        +dict rollout_stats_by_context

        +enqueue_prompt_groups(context, prompt_groups) void
        +step() ComponentResult | None
        +is_idle() bool
        +shutdown() void

        -collect_finished_tasks() int
        -submit_prompt_group_tasks() int
        -build_schedulable_context_states() list~RolloutContextState~
        -build_schedulable_context_state(context) RolloutContextState | None
        -pick_context(states) LoraContext | None

        -get_writable_partition(context) PartitionMetadata | None
        -get_open_partition_with_slot(context) PartitionMetadata | None
        -create_next_partition_if_allowed(context) PartitionMetadata | None
        -target_groups_for_context(context) int

        -submit_prompt_group_task(context) bool
        -pop_prompt_group(context) SampleRecord
        -run_prompt_group_rollout(group_ref, prompt_group) PartitionMetadata
        -invoke_rollout(trajectories, kwargs) list~SampleRecord~
        -compute_rewards(context, rollout_rows) list~float~ | None
    }

    class TransferQueueDataPlane {
        +get_open_partition(context) PartitionMetadata | None
        +create_open_partition(context, target_groups, policy_version) PartitionMetadata
        +seal_partition(context, partition_id) PartitionMetadata
        +partition_has_group_slot(partition) bool
        +start_prompt_group(context, partition, policy_version, adapter_path) PromptGroupRef
        +mark_prompt_group_running(group_ref) void
        +finish_prompt_group(group_ref, samples, rewards) PartitionMetadata
        +fail_prompt_group(group_ref, error) void
        +drop_prompt_group(group_ref, reason) void
        +list_groups(context) list~GroupMetadata~
    }

    class LoraAdapterRegistry {
        +get(context) AdapterRecord
        +register(context) AdapterRecord
        +can_accept_rollout(context) bool
        +on_partition_created(context, partition_id) void
        +on_rollout_started(context) void
        +on_rollout_finished(context) void
    }

    class StalenessManager {
        +can_open_next_partition(context, current_policy_version, groups) bool
        +is_group_too_stale(context, rollout_policy_version, current_policy_version) bool
    }

    class RolloutPolicy {
        +pick_next_context(states) LoraContext | None
    }

    class ToolManagerFactory {
        +create(prompt_group, context) ToolManager
    }

    class RolloutContextState {
        +LoraContext context
        +int pending_groups
        +int in_flight_rollouts
        +int live_partitions
        +int open_partitions
        +int train_ready_partitions
        +int rollout_capacity
        +float rollout_cost
        +float last_submit_time
        +int submitted_groups
        +float weight
    }
    ...
    AsyncRollouter --> TransferQueueDataPlane
    AsyncRollouter --> LoraAdapterRegistry
    AsyncRollouter --> StalenessManager
    AsyncRollouter --> RolloutPolicy
    AsyncRollouter --> ToolManagerFactory
    AsyncRollouter ..> RolloutContextState
    AsyncRollouter ..> PromptGroupRef
    AsyncRollouter ..> PartitionMetadata
```
