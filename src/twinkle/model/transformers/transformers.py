...
class TransformersModel(TwinkleModel, PreTrainedModel, CheckpointEngineMixin):
    ...
    def _build_micro_batch_plan(self, inputs, config, optimizer_config):
        ...
        while True:
            ...
            if optimizer_config._dp_group is None:
                return plan
            counts = [None] * dist.get_world_size(optimizer_config._dp_group)
            dist.all_gather_object(counts, len(plan), group=optimizer_config._dp_group)
            if all(count == len(plan) for count in counts):
                return plan
            min_micro_batches = max(counts)
            if min_micro_batches > len(inputs):
                raise ValueError(
