        ...
        for line in stream:
            ...
            for name, value in (record.get('values') or {}).items():
                ...
                if number is not None:
                    ...
                    values[f'{prefix}/{stage}/{_safe(metric_name)}'] = number
                    if smooth_span and stage == 'train' and name == 'reward':
                        ...
                        values[f'{prefix}/{stage}/reward_ema{smooth_span}'] = smoothed
            for field, name in (
                ('optimizer_step', 'train/optimizer_step'),
                ('policy_version', 'policy/version'),
                ('partition_index', 'partition/index'),
            ):
                number = _number(record.get(field))
                if number is not None:
                    values[f'{prefix}/{name}'] = number
            if not values:
                continue
            run.log(values, step=int(record['sequence']))
            logged += 1
    swanlab.finish()
    print(f'logged {logged} metric records to {logdir}')
    return logged
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='EMA span for train/reward; 0 disables smoothing',
    )
